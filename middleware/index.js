...
middlewareObj.checkParkOwenership = function(req, res, next) {
  if (req.isAuthenticated()) {
    Park.findById(req.params.id, (err, foundPark) => {
      if (err || !foundPark) {
        ...
        if (foundPark.author.id.equals(req.user._id) || req.user.isAdmin) { next(); }
        else {
          // add this message to next request
